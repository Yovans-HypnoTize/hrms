...
const DepartmentEntryDialog: React.FC<{
  showDialog: boolean;
  closeDialog: () => void;
  id?: number;
  reloadData: any;
  ...
  const [initialValue, setInitialValue] = useState<APIData.Department>(
    InitialData.Department
  );
  const [formDataSet, setFormDataSet] = useState(false);
  const [editMode, setEditMode] = useState(false);
  // const departmentAliveOptions: FormDataTypes.SelectOption[] = [
  //   { label: "Yes", value: "1" },
  //   { label: "No", value: "0" },
  // ];
  ...
  useEffect(() => {
    if (id !== undefined && id !== 0) {
      ...
      ServerAPI.getDepartmentDetail(id)
        .then((response) => {
          if (response) {
            setInitialValue({
              ...response,
              // company_department_status:
              //   response.company_department_status === true ? "1" : "0",
            });
            setFormDataSet(true);
            setEditMode(true);
          }
  ...
  const yupSchema = Yup.object().shape({
    ...
    company_department_name: Yup.string().required(
      ...
      {formDataSet && (
        <Formik
          ...
          onSubmit={(values, { setSubmitting }) => {
            ...
            if (id != undefined && id != 0) {
              // submitValues["company_department_status"] =
              //   submitValues.company_department_status === "1" ? true : false;
              addProcessingRequests();
              const keysToExtract = [
              ...
              ServerAPI.updateDepartment(updateValues, id)
                .then((response) => {
              ...
              //   submitValues.company_department_status === "1" ? true : false;
              addProcessingRequests();
              ServerAPI.addDepartment(submitValues)
                .then((response) => {
                  if (response && response["message"]) {
                    toast.success(response["message"]);
                    reloadData();
                      ...
                      <div className="col-lg-12 col-md-12 col-xl-12 text-field-empty-custom-user">
                        <FormField
                          ...
                          fieldProps={{
                            ...
                            name: "company_department_status",
                            options: departmentAliveOptions,
                            required: true,
                          }}
                        />
                      </div>
...
export default DepartmentEntryDialog;
