...
import { getDataFromAPI, getLoginClientID, getLoginCompanyID } from '../common/Utilities';
import { useAppStateAPI } from '../common/AppStateAPI';
import { API, ProjectStrings } from '../common/Constants';
import { ServerAPI } from '../common/ServerAPI';
import * as Yup from 'yup';

const PartnerEntryDialog: React.FC<{ showDialog: boolean, closeDialog: () => void, reloadData: any, id?: number, }> = ({ showDialog, closeDialog, reloadData, id }) => {
    const { addProcessingRequests, reduceProcessingRequests } = useAppStateAPI();
    const [initialValue, setInitialValue] = useState<APIData.Partner>(InitialData.Partner);
    ...
    useEffect(() => {
        if (id !== undefined && id !== 0) {
            ...
            ServerAPI.getPartnerDetail(id).then(response => {
                if (response) {
    ...
    const yupSchema = Yup.object().shape({
        partner_name: Yup.string().required(ProjectStrings.ValidationRequired),
        partner_address_line1: Yup.string().required(ProjectStrings.ValidationRequired),
        // partner_address_line2: Yup.string().required(ProjectStrings.ValidationRequired),
        partner_address_city: Yup.string().required(ProjectStrings.ValidationRequired),
        partner_address_postal_code: Yup.string().required(ProjectStrings.ValidationRequired),
        ...
        <Drawer anchor="right" open={showDialog} onClose={closeDialog} className="drawer-min">
            {formDataSet && (
                <Formik enableReinitialize={true} validateOnChange={false} validationSchema={yupSchema} initialValues={initialValue} onSubmit={(values, { setSubmitting }) => {
                    ...
                    submitValues['partner_address_postal_code'] = submitValues['partner_address_postal_code'].toString();
                    if (id != undefined && id != 0) {
                        addProcessingRequests();
                        ServerAPI.updatePartner(submitValues, id).then(response => {
                        ...
                        addProcessingRequests();
                        ServerAPI.addPartner(submitValues).then(response => {
                            if (response && response['message']) {
                                toast.success(response['message']);
                                reloadData();
                                closeDialog();
                    ...
                    {(formikProps) => {
                        if (formikProps.values.country_id && formikProps.values.country_id !== selectedCountryID) {
                            ...
                            console.log('country_id', formikProps.values.country_id);
                        }
                        return (
                            <div className="tabdetails">
                                {/* <h4 className="title-text text-center">{editMode ? 'Edit' : 'Add New'} Partner</h4> */}
                                <p className="detailed-heading mt-3">{editMode ? 'Edit' : 'Add New'} Partner</p>
                                    ...
                                    <div className="px-2 form-field-container">
                                        <div className="row px-2 py-3">
                                            <div className="col-lg-12 col-md-12 col-xl-12 text-field-empty-custom-user">
                                                <FormField formik={formikProps} fieldProps={{ fieldType: "textbox", label: "Partner Name", name: "partner_name", placeholder: "Enter Partner Name", required: true }} />
                                            </div>
                                        </div>
                                        <div className="row px-2 py-3">
                                            <div className="col-lg-12 col-md-12 col-xl-12 text-field-empty-custom-user">
                                                <FormField formik={formikProps} fieldProps={{ fieldType: "textbox", label: "Address Line 1", name: "partner_address_line1", placeholder: "Enter Company Address", required: true }} />
                                            </div>
                                        </div>
                                        <div className="row px-2 py-3">
                                            <div className="col-lg-12 col-md-12 col-xl-12 text-field-empty-custom-user">
                                                <FormField formik={formikProps} fieldProps={{ fieldType: "textbox", label: "Address Line 2", name: "partner_address_line2", placeholder: "Enter Company Address" }} />
                                            </div>
                                        </div>
                                        <div className="row px-2 py-3">
                                            <div className="col-lg-12 col-md-12 col-xl-12 text-field-empty-custom-user">
                                                <FormField formik={formikProps} fieldProps={{ fieldType: "textbox", label: "City Name", name: "partner_address_city", placeholder: "Enter City Name", required: true }} />
                                            </div>
                                        </div>
                                        <div className="row px-2 py-3">
                                            <div className="col-lg-12 col-md-12 col-xl-12 text-field-empty-custom-user">
                                        ...
                                        <div className="row px-2 py-3">
                                            <div className="col-lg-12 col-md-12 col-xl-12 text-field-empty-custom-user">
                                                <FormField formik={formikProps} fieldProps={{ fieldType: "number", label: "Zip / Postcode", name: "partner_address_postal_code", placeholder: "Enter Zip / Postcode", required: true }} />
                                            </div>
                                        </div>
                                        {/* <div className="row px-2 py-3">
                                            <div className="col-lg-12 col-md-12 col-xl-12 text-field-empty-custom-user">
...
export default PartnerEntryDialog;