...
const FormField: React.FC<{
  ...
  fieldProps: ThemeData.FormFieldProps;
}> = ({ formik, fieldProps }) => {
  ...
  const handleFileSelect = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (selectedFiles && selectedFiles.length > 0) {
      ...
      ServerAPI.uploadFileAndGetPath(selectedFile)
        .then((response: any) => {
          if (response) {
            let fileURL = response.data.url;
            console.log("response name", response.data.url);
            // formik.setFieldValue(otherProps.name, response["file_path"]);
            formik.setFieldValue(otherProps.name, fileURL);
            // const previewURL: string = response["preview_url"];
            const previewURL: string = fileURL;
            // const extn = previewURL?.includes("filename=")
            //   ? getFilenameExtension(previewURL.split("filename=")[1])
            //   : undefined;
            if (setPreviewImage) {
            ...
            setFieldPreviewImage(previewURL);
            setChangeFile(false);
          }
        })
        .finally(() => {
  ...
  if (fieldType === "textbox") {
        ...
        <Field
          ...
          inputProps={{
            ...
            style: {
              fontWeight: 400,
              color: "#252525",
              borderColor: otherProps.required ? "#d32f2f" : "#d32f2f",
            },
          }}
        >
          {initialValue}
    ...
    return (
      <div className="mb-24" key={selectKey}>
        <label htmlFor={otherProps.name} className="label-custom">
          {label}{" "}
          {otherProps.required && <span className="text-danger">*</span>}
        </label>
        <Select
          styles={customStyles}
          name={otherProps.name}
          ...
          defaultValue={options?.filter(
            (option) =>
              formik.getFieldProps(fieldProps.name).value + "" ==
              option.value + ""
          )}
          ...
          onChange={otherProps.onChange ? otherProps.onChange :(option: any) => {
    ...
    return (
      <div className="mb-24" key={selectKey}>
        ...
        <Select
          ...
          isDisabled={
            (formik.getFieldProps(fieldProps.name).value?.length || 0) >=
              maxOptions || readOnly
              ? true
              : false
          }
          defaultValue={(options ?? []).filter(
            (option) =>
              ...
              formik.getFieldProps(fieldProps.name).value &&
              formik.getFieldProps(fieldProps.name).value.includes(option.value)
          )}
          className="basic-multi-select"
        ...
        <div className="row">
          {formik.getFieldProps(fieldProps.name) &&
            formik.getFieldProps(fieldProps.name).value &&
            Array.isArray(formik.getFieldProps(fieldProps.name).value) &&
            formik
                              ...
                              onClick={() => {
                                formik.setFieldValue(
                                  ...
                                  formik
                                    ...
                                    .value.filter(
                                      (optValue: string) =>
                                        optValue !== selectedOption[0].value
                                    )
                                );
                                setSelectKey(Math.random());
                              }}
      ...
      <div className="mb-24">
        ...
        <div>
          <DatePicker
            ...
            selected={
              formik.values[otherProps.name]
                ? new Date(formik.values[otherProps.name])
                : null
            }
            onChange={(date) => {
            ...
            maxDate={otherProps.customAllowDate ? otherProps.customAllowDateValue : otherProps.allowFutureDate ? futureDate : currentDate} // Conditionally set maxDate
          />
        </div>
        <ErrorMessage
          component={FormHelperText}
  ...
  } else if (fieldType === "hourtimepicker") {
    return (
      <div className="mb-24">
        <label htmlFor={otherProps.name} className="label-custom">
          {label}{" "}
        ...
        <div>
          <LocalizationProvider dateAdapter={AdapterDayjs}>
            <TimePicker
              ...
              format="H" // Display format
              value={
                formik.getFieldProps(fieldProps.name).value
                  ? dayjs()
                      .hour(formik.getFieldProps(fieldProps.name).value)
                      .minute(0)
                  : null
              }
              onChange={(date) => {
                if (date) {
                  ...
                  formik.setFieldValue(otherProps.name, selectedHour); // Store only hour (8, 9, etc.)
                  formik.setFieldTouched(otherProps.name, true);
                }
              }}
              ampm={!otherProps.timepicker24HFormat} // 12H or 24H based on props
            />
          </LocalizationProvider>
        </div>
        <ErrorMessage
    ...
    if (options !== undefined) {
      const filtered = options.filter(
        (option) => option.value == formik.getFieldProps(fieldProps.name).value
      );
    ...
    return (
      ...
      <div className="mb-24">
        ...
        <div
          ...
          aria-labelledby={otherProps.name + "-group"}
        >
          {options &&
            options.map((option, index) => (
              <label
                className="mr-3 side-heading-label d-flex align-items-center"
                key={index}
              >
                <Field
                  type="radio"
                  ...
                  value={option.value}
                  checked={
                    formik.getFieldProps(fieldProps.name).value == option.value
                      ? true
                      : false
                  }
                  readOnly={readOnly ? true : false}
  ...
  } else if (fieldType === "radioGroupOptions") {
    return (
      // <div className="mb-24 d-flex flex-wrap">
      <div className="mb-24">
        <label htmlFor={otherProps.name} className="form-label side-headings">
      ...
      <div className="mb-24">
        <InputLabel>
          <Field
            ...
            checked={
              formik.getFieldProps(fieldProps.name).value &&
              formik.getFieldProps(fieldProps.name).value !== "0"
                ? true
                : false
            }
            readOnly={readOnly ? true : false}
            ...
            onChange={() => {
              if (!readOnly) {
                formik.setFieldValue(
                  otherProps.name,
                  formik.getFieldProps(fieldProps.name).value ? 0 : 1
                );
              }
            }}
          />
        ...
        <div className="toggle-switch">
          <label className="toggle-label">
            <input
              ...
              checked={
                formik.getFieldProps(fieldProps.name).value ? true : false
              }
              onChange={() => {
                if (!readOnly) {
                  formik.setFieldValue(
          ...
          <label
            className="custom-control-label text-muted table-text"
    ...
    return (
      <div className="mb-24">
        <InputLabel htmlFor={otherProps.name} className="form-upload-label">
          {label}
        </InputLabel>
        {uploading && (
          <img
            src={window.location.origin + "/assets/images/uploading.gif"}
          ...
          !changeFile && (
            <>
              ...
              <button
                ...
                onClick={(e) => {
                  console.log(formik.getFieldProps(fieldProps.name).value)
                  formik.setFieldValue(fieldProps.name,"")
                  e.preventDefault();
                  setChangeFile(true);
                }}
              >
                <i
                  className="fas fa-trash text-info font-16"
                  aria-hidden="true"
                ></i>
              </button>
              {/* <a
        ...
        {!uploading &&
          ...
          !changeFile && (
            <>
              ...
              >
                <i
                    className="fas fa-edit text-info font-16"
                    aria-hidden="true"
                  ></i>
              </button>
                // <a
        ...
        {!uploading &&
          ((!formik.getFieldProps(fieldProps.name).value && !previewImage) ||
            changeFile ||
            (formik.getFieldProps(fieldProps.name).value === null &&
              previewImage === "null")) && (
            <>
              <div
