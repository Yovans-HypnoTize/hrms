...
import { Endpoints, StatusValues } from "../../common/Constants";
...
const SubordinatePendingLeaveRequest: React.FC = () => {
    ...
    const updateSubordinateRequest = (id: number, status: number, remarks: string) => {
        addProcessingRequests();
        ServerAPI.subordinateleaveRequestUpdate(id, status, remarks).then(response => {
    ...
    useEffect(() => {
        getSubordinateLeave();
        setReloadData(false);
    }, [dateRangeFrom, dateRangeTo, searchString, seletedLeaveType, seletedStatus, reloadData])
    ...
    return (
        <div className='subordinate mt-1'>
            <div className='col-12 d-flex justify-content-start align-items-center p-0 mt-3 flex-wrap' style={{ gap: "15px" }}>
                <h4 className="header-title mt-0 mb-0">Pending Leave Request</h4>
                <RangePicker
            ...
            {/* <div className='col-6 p-0'>
                        <div className='filter-option d-flex gap-2 p-2 mt-2'>
                            <button className='mr-1'><img src="/assets/svg/filter-circle.svg" alt="circle-icon" className='mr-1 mt-0' height={'14px'} /> Filter By</button>
                            <div className='mr-1'> <button className='select' onClick={() => setShowoption(!showOption)}>Employee <i className={`mdi ${showOption ? 'mdi-chevron-up' : 'mdi-chevron-down'}`}></i></button>
                            ...
                            <div className='mr-1'> <button className='select'>Date <i className={`mdi ${showOption ? 'mdi-chevron-up' : 'mdi-chevron-down'}`}></i></button></div>
                            <div className='mr-1'> <button className='select'>Period <i className={`mdi ${showOption ? 'mdi-chevron-up' : 'mdi-chevron-down'}`}></i></button></div>
                            <div className='mr-1'> <button className='select'>Leave Type<i className={`mdi ${showOption ? 'mdi-chevron-up' : 'mdi-chevron-down'}`}></i></button></div>
                        </div>
                    </div> */}
                    ...
                    <div className="table-container subordinate-leave" key={leaveIDX}>
                        <div className="table-scroll">
                            <div className="table-responsive">
                                <div className='d-flex align-items-center p-2' style={{ gap: "10px" }}>
                                ...
                                <div className="row btn-form-submit justify-content-between m-2" style={{ gap: "10px" }}>
                                    <button type="button" className="button1 bg-danger text-white w-40 p-2" style={{ width: "48%" }} onClick={() => { updateSubordinateRequest(leave.employee_leave_req_id, StatusValues.Rejected, leave.employee_leave_req_status_remarks); setReloadData(true); }}>Reject</button>
                                    <button type="button" className="button2 bg-success text-white w-40 p-2" style={{ width: "48%" }} onClick={() => { updateSubordinateRequest(leave.employee_leave_req_id, StatusValues.Approved, leave.employee_leave_req_status_remarks); setReloadData(true); }}>Accept</button>
                                </div>
                            </div>
                        </div>
                    </div>
...
export default SubordinatePendingLeaveRequest;